                    ...
                    <div className="p-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30 flex justify-between items-start">
                        <div className="flex-1">
                            ...
                            <div className="flex flex-wrap gap-2">
                                <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-50 text-indigo-600 border border-indigo-200 dark:bg-indigo-500/10 dark:text-indigo-400 dark:border-indigo-500/20 text-sm font-medium"><Smile size={14} /> {t('mood_' + selectedEntry.analysis.mood)}</span>
                                <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-100 text-slate-600 border border-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700 text-sm">Score: {selectedEntry.analysis.sentimentScore.toFixed(2)}</span>
                                {selectedEntry.location && (<a href={`https://www.google.com/maps/search/?api=1&query=${selectedEntry.location.lat},${selectedEntry.location.lng}`} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 text-emerald-600 border border-emerald-200 hover:bg-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20 dark:hover:bg-emerald-500/20 text-sm transition-colors"><MapPin size={14} /> {selectedEntry.city ? `${selectedEntry.city}, ${selectedEntry.country}` : 'Location'}</a>)}
                            </div>
                        </div>
